...
import { addEVMVerifierAddress, removeEVMVerifierAddress } from 'server/store';
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (request.method === 'POST') {
    ...
    await addEVMVerifierAddress(data.chainId, data.address);
  }
  if (request.method === 'DELETE') {
    ...
    await removeEVMVerifierAddress(data.chainId);
  }
  return null;
}
...
export async function loader() {
  ...
  return {
    chains,
    customVerifiers,
    defaultVerifiers: Object.entries(defaultEVMVerifiers).map(
      ([chainId, address]) => ({
        chainId,
        address,
      })
...
export default function EVMVerifiers() {
